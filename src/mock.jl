macro mock(expr)
    ...
    # Due to the fix for Julia issue #265 (PR #17057) when `Mocking.activated` is
    # overwritten then recompilation will be triggered on all dependent functions.
    # When `Mocking.activated() == false` then Julia will optimize the code below to have
    # zero-overhead by only executing the original expression.
    result = quote
